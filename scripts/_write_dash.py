"""Helper: overwrite agent/dashboard.py with the full tabbed dashboard."""
import pathlib, sys
...
async function initOverview(){
  try{
    ...
    document.getElementById('ov-stats').innerHTML=cards.map(c=>
      ...
      c.stats.map(s=>'<div class="stat"><span class="sl">'+s.l+'</span><span class="sv '+(s.c||'")>'+s.v+'</span></div>').join('')+
      '</div>'
    ).join('');
  }catch(e){document.getElementById('srv-status').innerHTML='<span class="fail">✗ offline</span>';}
  try{
    const r=await apiFetch('/jobs');const d=await r.json();
    const jobs=d.jobs||[];
    document.getElementById('ov-jobs').innerHTML=jobs.length?
      jobs.map(j=>'<div class="stat"><span class="sl">'+(j.id||j.name)+'</span><span class="sv info">'+(j.next_run||j.status||'—')+'</span></div>').join(''):
...
async function loadChatModels(){
  try{
    ...
    sel.innerHTML='<option value="">Auto-route</option>'+(d.models||[]).map(m=>'<option value="'+m.id+'">'+m.id+' ('+m.tier+')</option>').join('');
  }catch(e){}
}
async function sendChat(){
  ...
  try{
    const body={message:txt,session_id:session};if(model)body.model=model;
...
async function loadHistory(){
  ...
  try{
    const r=await apiFetch('/history/'+sid);const d=await r.json();
...
async function clearHistory(){
  const sid=document.getElementById('chat-session').value||'demo';
  try{
    await apiFetch('/history/'+sid,{method:'DELETE'});
    document.getElementById('chat-hist').textContent='Cleared.';
  }catch(e){document.getElementById('chat-hist').textContent=String(e);}
}
...
async function loadRagDocs(){
  try{
    const r=await apiFetch('/rag/documents');const d=await r.json();
...
async function runSandbox(){
  ...
  try{
    const r=await apiFetch('/sandbox/execute',{method:'POST',body:JSON.stringify({code,language,timeout})});
    const d=await r.json();
...
async function kgSearch(){
  ...
  try{
    const r=await apiFetch('/kg/search?q='+encodeURIComponent(q));
    const d=await r.json();
...
async function kgPath(){
  ...
  try{
    const r=await apiFetch('/kg/path',{method:'POST',body:JSON.stringify({from_node:from,to_node:to})});
    const d=await r.json();
    document.getElementById('kg-search-out').textContent=JSON.stringify(d,null,2);
  }catch(e){document.getElementById('kg-search-out').textContent=String(e);}
}
...
async function loadToolSchema(fmt){
  try{
    const r=await apiFetch('/tools/schema?format='+fmt);const d=await r.json();
...
async function callTool(){
  ...
  try{
    const r=await apiFetch('/tools/call',{method:'POST',body:JSON.stringify({name,params})});
    const d=await r.json();
...
async function runVision(){
  ...
  if(!image)return;
  try{
    const r=await apiFetch('/vision',{method:'POST',body:JSON.stringify({image,prompt})});
    const d=await r.json();
...
async function fetchModels(){
  try{
    ...
    const rows=(d.models||[]).map(m=>
      ...
      '<td style="color:var(--blue);text-align:right">'+(m.context_k||'—')+'k</td>'+
      '<td style="color:var(--muted);font-size:0.75rem">'+(m.best_for||'')+'</td>'+
      '<td style="font-size:0.72rem">'+(m.capabilities||[]).map(c=>'<span class="bdg bdg-p">'+c+'</span>').join(' ')+'</td>'+
      '</tr>'
    ).join('');
...
async function setPersona(){
  ...
  try{
    const r=await apiFetch('/personas/set',{method:'POST',body:JSON.stringify({persona_id:id})});
    const d=await r.json();
...
async function loadMemories(){
  try{
    const r=await apiFetch('/memory/list');const d=await r.json();
    const ms=d.memories||[];
...
final = MODULE_HEADER + DASHBOARD_HTML + '"""\n' + REGISTER_FOOTER

# Project root is 1 level up from scripts/
target = pathlib.Path(__file__).parent.parent / "agent" / "dashboard.py"
target.write_text(final, encoding="utf-8")
print(f"Written {len(final):,} chars to {target}")
